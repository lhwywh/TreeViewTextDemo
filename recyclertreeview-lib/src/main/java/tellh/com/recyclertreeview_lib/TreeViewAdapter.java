...
public class TreeViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public TreeViewAdapter(List<TreeNode> nodes, List<? extends TreeViewBinder> viewBinders) {
        ...
        for(int i = 0;i<viewBinders.size();i++){
            isClicks.add(false);
        }
    }
...
//               }
//               holder.itemView.setTag(System.currentTimeMillis());
               if (onTreeNodeListener !=null &&onTreeNodeListener.onLongClick(selectedNode,holder,position))
                   return true;
               return  true;
           }
       });
    ...
    public int removeChildNodes(TreeNode pNode) {
        return removeChildNodes(pNode, true);
    }

    public int removeChildNodes(TreeNode pNode, boolean shouldToggle) {
        if (pNode.isLeaf())
            return 0;
        List<TreeNode> childList = pNode.getChildList();
    ...
    public interface OnTreeNodeListener {
         ...
         */
        boolean onLongClick(TreeNode node, RecyclerView.ViewHolder holder,int position);
    ...
    public List<TreeNode> getDisplayNodesList(){
        return  displayNodes;
    }

    public List<TreeNode> getDeletedDisplayNodesList(TreeNode deleteNode){
        while(displayNodes.indexOf(deleteNode)!=-1) {
            notifyItemRemoved(displayNodes.indexOf(deleteNode));
            displayNodes.remove(deleteNode);
        }
        notifyDataSetChanged();
        return displayNodes;
    }
